...
const findPermissionForRoute = (policies, route, prefix = '') => {
  ...
  const permissionRoute = policies[0].permissions.filter((p) => {
    ...
    routeArray.forEach((item, i) => {
      if ((item !== accessuri[i] && accessuri[i] !== '*' && accessuri[i] !== undefined)
        || (routeArray.length < accessuri.length) || (accessuri[i] === undefined && accessuri[accessuri.length - 1] !== '*')) {
        hasPermission = false;
      }
    });
...
const deny = (customMessage) => {
  return { code: 403, message: customMessage || 'Unauthorized access' };
};
