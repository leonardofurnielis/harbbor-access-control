...
const accessPoliciesSchema = Joi.object().keys({
...
const accessGroupSchema = Joi.object().keys({
  ...
  permissions: Joi.array().items(accessPoliciesSchema.required()),
});
...
function validate(access_policies) {
  const cwt = schema.validate(access_policies);
  if (cwt.error) {
    throw TypeError(JSON.stringify(cwt.error.message));
  }
}
...
module.exports.validate = (params) => {
  if (params && typeof params.access_policies === 'function') {
    console.warn(
  ...
  if (params && params.access_policies[0]) {
